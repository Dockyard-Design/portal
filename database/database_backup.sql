...
-- Enable RLS on all tables and allow authenticated access
-- ============================================
...
-- For authenticated users (matches your use case where Clerk handles auth)

-- Projects
CREATE POLICY "Enable all operations for authenticated users" ON projects FOR ALL USING (auth.role() = 'authenticated') WITH CHECK (auth.role() = 'authenticated');

-- API Keys  
CREATE POLICY "Enable all operations for authenticated users" ON api_keys FOR ALL USING (auth.role() = 'authenticated') WITH CHECK (auth.role() = 'authenticated');

-- Settings (read-only for most)
CREATE POLICY "Enable all operations for authenticated users" ON settings FOR ALL USING (auth.role() = 'authenticated') WITH CHECK (auth.role() = 'authenticated');
...
-- Kanban Boards
CREATE POLICY "Enable all operations for authenticated users" ON kanban_boards FOR ALL USING (auth.role() = 'authenticated') WITH CHECK (auth.role() = 'authenticated');

-- Kanban Tasks
CREATE POLICY "Enable all operations for authenticated users" ON kanban_tasks FOR ALL USING (auth.role() = 'authenticated') WITH CHECK (auth.role() = 'authenticated');

-- Quotes
CREATE POLICY "Enable all operations for authenticated users" ON quotes FOR ALL USING (auth.role() = 'authenticated') WITH CHECK (auth.role() = 'authenticated');

-- Quote Items
CREATE POLICY "Enable all operations for authenticated users" ON quote_items FOR ALL USING (auth.role() = 'authenticated') WITH CHECK (auth.role() = 'authenticated');

-- Invoices
CREATE POLICY "Enable all operations for authenticated users" ON invoices FOR ALL USING (auth.role() = 'authenticated') WITH CHECK (auth.role() = 'authenticated');

-- Invoice Items
CREATE POLICY "Enable all operations for authenticated users" ON invoice_items FOR ALL USING (auth.role() = 'authenticated') WITH CHECK (auth.role() = 'authenticated');

-- Expenses
CREATE POLICY "Enable all operations for authenticated users" ON expenses FOR ALL USING (auth.role() = 'authenticated') WITH CHECK (auth.role() = 'authenticated');

-- Expense Categories
CREATE POLICY "Enable all operations for authenticated users" ON expense_categories FOR ALL USING (auth.role() = 'authenticated') WITH CHECK (auth.role() = 'authenticated');

-- Contact Submissions
CREATE POLICY "Enable all operations for authenticated users" ON contact_submissions FOR ALL USING (auth.role() = 'authenticated') WITH CHECK (auth.role() = 'authenticated');

-- API Request Logs (insert only from service role typically, but allow for now)
CREATE POLICY "Enable all operations for authenticated users" ON api_request_logs FOR ALL USING (auth.role() = 'authenticated') WITH CHECK (auth.role() = 'authenticated');

-- Rate Limits
CREATE POLICY "Enable all operations for authenticated users" ON api_rate_limits FOR ALL USING (auth.role() = 'authenticated') WITH CHECK (auth.role() = 'authenticated');
